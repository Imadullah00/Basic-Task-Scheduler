...
//DESCRIPTION: This is a basic task scheduler. I like to call it a primitive version off an OS.
//			   It switches between tasks ofturning on different EDs  with some programmable delay
...
//			   to perform the context seitching
...
void task_delay(uint32_t tick_count)
{
	...
	if(current_task)
	{
	   user_tasks[current_task].block_count = global_tick_cnt + tick_count;
...
void unblock_tasks(void)
{
	for(int i = 1 ; i < MAX_TASKS ; i++)
	{
		...
		{
			if(user_tasks[i].block_count == global_tick_cnt)
			{
